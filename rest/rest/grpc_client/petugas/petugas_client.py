import rest.grpc_client.petugas.petugas_pb2_grpc as petugas_pb2_grpc
import rest.grpc_client.petugas.petugas_pb2 as petugas_pb2
import grpc
import traceback
...
class PetugasClient:
    ...
    def create_petugas(self, fnama, email, password, username):
        response = self.stub.Create(
            petugas_pb2.PetugasCreateRequest(
                fnama=fnama,
                email=email,
                password=password,
                username=username,
    ...
    def update_petugas(self, id, fnama, email, password, username, status):
        response = self.stub.Update(
            petugas_pb2.PetugasUpdateRequest(
                id=id,
                fnama=fnama,
                email=email,
                password=password,
    ...
    def login_petugas(self, email, password):
        response = self.stub.Login(
            petugas_pb2.PetugasLoginRequest(email=email, password=password)
        )
        ...
        return dict(token=response.token, email=response.email, fnama=response.fnama)
